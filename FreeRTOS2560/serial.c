...
{
unsigned long ulBaUDR0ateCounter;
unsigned char ucByte;
	...
	portENTER_CRITICAL();
	{
		...
		data sheet. */
		ulBaUDR0ateCounter = ( configCPU_CLOCK_HZ / ( serBAUD_DIV_CONSTANT * ulWantedBaud ) ) - ( unsigned long ) 1;

		/* Set the baud rate. */	
		ucByte = ( unsigned char ) ( ulBaUDR0ateCounter & ( unsigned long ) 0xff );	
		UBRR0L = ucByte;

		ulBaUDR0ateCounter >>= ( unsigned long ) 8;
		ucByte = ( unsigned char ) ( ulBaUDR0ateCounter & ( unsigned long ) 0xff );	
		UBRR0H = ucByte;
